...
from transformers.models.qwen3_moe.modeling_qwen3_moe import Qwen3MoeDecoderLayer, Qwen3MoeMLP

from .configuration_ernie4_5_moe_nonuniform import Ernie4_5_MoeConfig
...
class Ernie4_5_MoeStatics(nn.Module):
    ...
    def __init__(self, config, num_experts: Optional[int] = None):
        ...
        num_experts = config.moe_num_experts if num_experts is None else int(num_experts)
...
class Ernie4_5_MoeExperts(MixtralExperts):
    def __init__(self, config, num_experts: Optional[int] = None):
        super().__init__()
        self.num_experts = config.moe_num_experts if num_experts is None else int(num_experts)
...
class Ernie4_5_MoeTopKRouter(nn.Module):
    def __init__(self, config, num_experts: Optional[int] = None):
        super().__init__()
        self.num_experts = config.moe_num_experts if num_experts is None else int(num_experts)
        self.weight = nn.Parameter(torch.zeros(self.num_experts, config.hidden_size, dtype=torch.float32))
        self.moe_statics = Ernie4_5_MoeStatics(config, num_experts=self.num_experts)
        self.top_k = min(int(config.moe_k), int(self.num_experts))
...
class Ernie4_5_MoeSparseMoeBlock(nn.Module):
    def __init__(self, config, num_experts: Optional[int] = None):
        ...
        self.num_experts = config.moe_num_experts if num_experts is None else int(num_experts)
        self.top_k = min(int(config.moe_k), int(self.num_experts))
        self.gate = Ernie4_5_MoeTopKRouter(config, num_experts=self.num_experts)
        self.experts = Ernie4_5_MoeExperts(config, num_experts=self.num_experts)
...
class Ernie4_5_MoeDecoderLayer(Qwen3MoeDecoderLayer):
    def __init__(self, config, layer_idx):
        ...
        if (
            ...
            num_experts = (
                config.get_num_experts(layer_idx) if hasattr(config, "get_num_experts") else config.moe_num_experts
            )
            self.mlp = Ernie4_5_MoeSparseMoeBlock(config, num_experts=num_experts)
        else:
...
class Ernie4_5_MoeForCausalLM(MixtralForCausalLM):
    def __init__(self, config):
        ...
        self.num_experts = (
            max(config.num_experts_per_layer) if getattr(config, "num_experts_per_layer", None) else config.moe_num_experts
        )
