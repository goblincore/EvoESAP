...
from .configuration_ernie4_5_moe_nonuniform import Ernie4_5_MoeConfig, NonUniformErnie4_5_MoeConfig
...
class Ernie4_5_MoeSparseMoeBlock(nn.Module):
    ...
    # @property
    # def moe_statics(self) -> nn.Module:
    #     # Compatibility alias: some loaders (e.g., vLLM Transformers backend) expect
    #     # `model.layers.*.mlp.moe_statics.*` to resolve to the correction module.
    #     return self.gate.moe_statics
...
class Ernie4_5_MoeDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx):
        ...
        if (
            ...
            num_experts = config.get_num_experts(layer_idx) if hasattr(config, "get_num_experts") else config.moe_num_experts
...
def load_balancing_loss_func(
    ...
    per_layer_num_experts = [layer_gate.shape[-1] for layer_gate in gate_logits]

    # Preserve upstream behavior when all layers share the same expert dimension.
    if len(set(per_layer_num_experts)) == 1:
        # Always trust the logits shape (pruning may have changed expert dims without re-instantiating the wrapper).
        num_experts = int(per_layer_num_experts[0])
    ...
    # Non-uniform expert dimension across layers: compute per-layer loss and average.
    losses = []
    if attention_mask is None:
        for layer_gate in gate_logits:
            layer_gate = layer_gate.to(compute_device)
            layer_num_experts = layer_gate.shape[-1]
            layer_top_k = min(int(top_k), int(layer_num_experts))
            routing_weights = torch.nn.functional.softmax(layer_gate, dim=-1)
            _, selected_experts = torch.topk(routing_weights, layer_top_k, dim=-1)
            expert_mask = torch.nn.functional.one_hot(selected_experts, layer_num_experts)
            tokens_per_expert = torch.mean(expert_mask.float(), dim=0)
            router_prob_per_expert = torch.mean(routing_weights, dim=0)
            losses.append(torch.sum(tokens_per_expert * router_prob_per_expert.unsqueeze(0)) * layer_num_experts)
    else:
        batch_size, sequence_length = attention_mask.shape
        for layer_gate in gate_logits:
            layer_gate = layer_gate.to(compute_device)
            ...
            expert_attention_mask = (
                attention_mask[:, :, None, None]
                .expand((batch_size, sequence_length, layer_top_k, layer_num_experts))
                .reshape(-1, layer_top_k, layer_num_experts)
                .to(compute_device)
            )
            tokens_per_expert = torch.sum(expert_mask.float() * expert_attention_mask, dim=0) / torch.sum(
                expert_attention_mask, dim=0
            ...
            router_per_expert_attention_mask = (
                attention_mask[:, :, None]
                .expand((batch_size, sequence_length, layer_num_experts))
                .reshape(-1, layer_num_experts)
                .to(compute_device)
            )
            router_prob_per_expert = torch.sum(
                routing_weights * router_per_expert_attention_mask, dim=0
...
class NonUniformErnie4_5_MoePreTrainedModel(Ernie4_5_MoePreTrainedModel):
    config_class = NonUniformErnie4_5_MoeConfig


class NonUniformErnie4_5_MoeModel(Ernie4_5_MoeModel):
    config_class = NonUniformErnie4_5_MoeConfig


class NonUniformErnie4_5_MoeForCausalLM(Ernie4_5_MoeForCausalLM):
    config_class = NonUniformErnie4_5_MoeConfig
...
__all__ = [
    ...
    "NonUniformErnie4_5_MoePreTrainedModel",
]
